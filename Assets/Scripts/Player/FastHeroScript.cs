...
public class FastHeroScript : MonoBehaviour, IHero
{
    ...
    private const float smallShipColliderX = 0.98f;
    private const float smallShipColliderY = 0.74f;
    ...
    [SerializeField] private float missileXOffset;
    ...
    public void ChangeToHero(IHero previousHero)
    {
        ...
        playerScript.boxCollider.size = new Vector2(smallShipColliderX, smallShipColliderY);
    }
    ...
    {
        Missile tempMissile = Instantiate(missile, transform.position + (transform.right * missileXOffset), transform.rotation).GetComponent<Missile>();
        Missile tempMissile2 = Instantiate(missile, transform.position - (transform.right * missileXOffset), transform.rotation).GetComponent<Missile>();
        tempMissile.damage = damagerPerAttack;
        tempMissile.lifetime = missileLifetime;
        tempMissile.speed = missileSpeed;
        tempMissile2.damage = damagerPerAttack;
        tempMissile2.lifetime = missileLifetime;
        tempMissile2.speed = missileSpeed;
    ...
    public IEnumerator HeroAbility()
    {
        ...
        playerScript.dashDirection = movementScript.GetMovementDirection();
        playerScript.playerDashing = true;
        playerScript.movementSpeed = dashSpeed;
        playerScript.audioSource.PlayOneShot(dashSFX);
        yield return new WaitForSeconds(abilityDuration);
        playerScript.movementSpeed = movementSpeed;
        abilityCooling = true;
        StartCoroutine(AbilityCooldown());
        playerScript.playerDashing = false;
